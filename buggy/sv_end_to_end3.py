...
def find_matches(pic1, pic2):
    ...
    cv2.imwrite('/Users/svutukur/Documents/GitHub/cv_mdm2019/' + 'imageswMatches.png', img3)
    
    #return homography transform matrix H and image coordinate displacement vector
    return [H, img1_pts, img2_pts] #change name; what *type* of displacements are these
...
while first < num_images-1:
    [H, img1_pts, img2_pts] = find_matches(images[first], images[first+1])
    ...
    #get physical coord matrices w1, w2 from homography transformation
    w1 = []
    w2 = []
    
    for i in range(len(img1_pts)):
        w1.append(np.matmul(inv(H), img1_pts[i])) #w1 physical coord
        w2.append(np.matmul(inv(H), img2_pts[i])) #w2 physical coord
