...
def corner_detector(list_names):
    ...
    old_frame = old_frame[x1:x2, y1:y2] #coordinates from trials; crop picture to do detection and tracking on
    old_gray = old_gray[x1:x2, y1:y2]
...
#    old_gray = old_gray[]
    ...
    while index < (len(list_names)-1):
        ...
        img = cv2.add(frame,mask); #flow
        img = cv2.add(old_frame = cv2.imread(list_names[index]),img) #OG image + flow
        img = cv2.add(img,boundbox) #OG + flow + ROI rectangle, boundbox from trials
        img = cv2.resize(img, (960, 540))  #resize
