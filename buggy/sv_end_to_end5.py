...
def load_images():
    ...
    for filename in sorted(files):
      case = fnmatch.fnmatchcase(filename, 'cam1_0000*.tif')
      if case == True:
...
if __name__ == "__main__":
    [newcameramtx, total_error] = sv_calibrate2.main() #extract the camera distorion matrix
    
    images = ['C:/Users/svutukur/Documents/tbw1_data/A_Run143_Seq' + str(i) + '_00001.tif' for i in range(6,9)]
    
    disp3 = []
    first = 0
    
    while first < len(images)-1:
        [H, img1_pts, img2_pts, img3] = find_matches(images[0], images[first+1])
        ...
        cv2.imwrite('/Users/svutukur/Documents/GitHub/cv_mdm2019/' + 'Matches' + str(0) + '-' + str(first+1) +'.png', img3)
        print('completed' + ' ' + images[0], images[first+1])
        first = first + 1;
